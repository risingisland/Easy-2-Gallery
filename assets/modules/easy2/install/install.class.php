...
class install {
    ...
    public function restore($path, $pid) {
         ...
         */
        // mysqli Dir list
        $res = mysqli_query($this->modx->db->conn,'SELECT cat_id,cat_name,parent_id FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_dirs WHERE parent_id=' . $pid);
    ...
    public function restoreAll($path, $pid) {
        ...
        if( $res ){
            ...
            $_SESSION['easy2err'][] = __LINE__ . ': ' . 'database query ERROR: SELECT cat_id,cat_name from easy2_dirs';
            return FALSE;
    ...
    public function databaseCharSet($databaseCollation, $variable) {
        ...
        $getCol = mysqli_query($this->modx->db->conn,"SHOW COLLATION");
        $showVars = $this->modx->db->makeArray($this->modx->db->query("SHOW VARIABLES"));
        foreach ($showVars as $v) {
            $mysqliVars[$v['Variable_name']] = $v['Value'];
        }
        $databaseCollation = $mysqliVars['collation_database'];
...
/*        if (!empty($post['plugin_id'])) {
            ...
            if ($delete) {
                foreach ($nEvtIds as $nEvtId) {
                    $this->modx->db->query( 'INSERT INTO ' . $this->modx->db->config['table_prefix'] . 'site_plugin_events '
