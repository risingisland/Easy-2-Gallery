...
class E2gMod extends E2gPub {
    ...
    private function _galleryPath() {
        ...
        if (!empty($this->sanitizedGets['path']) && $path['string'] !== $this->sanitizedGets['path']
        ) {
    ...
    public function explore() {
        ...
        switch ($act) {
            ...
            case 'create_dir':
                ...
                if ($this->_hasBadChar($_POST['name'], __LINE__) || !$this->_createDir($_POST, $this->e2gModCfg['gdir'], $this->e2gModCfg['parent_id'])
                ) {
                    header('Location: ' . html_entity_decode($_SERVER['HTTP_REFERER'], ENT_NOQUOTES));
                } else {
            ...
            case 'save_dir':
                // check names against bad characters
                if ($this->_hasBadChar($_POST['newdirname'], __LINE__) || !$this->_editDir($_POST)
                ) {
                    header('Location: ' . html_entity_decode($_SERVER['HTTP_REFERER'], ENT_NOQUOTES));
            ...
            case 'save_file':
                // check names against bad characters
                if ($this->_hasBadChar($_POST['newfilename'], __LINE__) || !$this->_editFile($this->e2gModCfg['gdir'], $_POST)
                ) {
                    header('Location: ' . html_entity_decode($_SERVER['HTTP_REFERER'], ENT_NOQUOTES));
                } elseif (isset($this->sanitizedGets['tag'])) {
                    header('Location: ' . html_entity_decode($this->e2gModCfg['index'] . '&amp;tag=' . $this->sanitizedGets['tag']));
    ...
    private function _addFile($filePath, $pid, $userId = null) {
        ...
        if (empty($fileRealPath)) {
            $err = __LINE__ . ' : ' . $this->lng['file_add_err'] . ': ' . $fileRealPath;
            $this->setError($err);
            return FALSE;
        }
        if (empty($userId)) {
            $err = ___LINE__ . ' : ' . $this->lng['invalid_uid'];
            $this->setError($err);
            return FALSE;
        }
        ...
        if ($inf[2] > 3 || !is_numeric($pid)) {
            $err = __LINE__ . ' : ' . $this->lng['file_add_err'];
            $this->setError($err);
            return FALSE;
        }
        ...
        if ($basename != $fileAlias) {
            ...
            if (!$rename) {
                $err = __LINE__ . ' : ' . __LINE__ . ' : ' . $this->lng['file_rename_err'] . $filePath . ' => ' . $dirPath . DIRECTORY_SEPARATOR . $this->e2gDecode($fileAlias);
                $this->setError($err);
                return FALSE;
            }
        ...
        $insertFile = 'INSERT INTO ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                ...
                . ", modified_by='$userId'"
        ;
        $queryInsertFile = mysqli_query($this->modx->db->conn,$insertFile);
        if (!$queryInsertFile) {
            $err = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertFile;
            $this->setError($err);
            return FALSE;
        }
    ...
    private function _resizeImg($filename, $inf, $w, $h, $thq) {
        ...
        $newInf['time'] = date("Y-m-d H:i:s", $lastModified);
...
//                return FALSE;
    ...
    public function synchro($path, $pid, $userId = null) {
        ...
        if (!$this->validFolder($path) || empty($path)) {
            $err = __LINE__ . ' : ' . $this->lng['err_empty_path'] . ' $path=' . $path;
            $this->setError($err);
            return FALSE;
        }
        if (empty($pid)) {
            $err = __LINE__ . ' : ' . $this->lng['err_empty_id'] . ' $pid=' . $pid;
            $this->setError($err);
            return FALSE;
        }
        if (empty($userId)) {
            $err = __LINE__ . ' : ' . $this->lng['invalid_uid'] . ' : ' . $userId;
            $this->setError($err);
            return FALSE;
        }
        ...
        $selectDirs = 'SELECT * FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_dirs '
                . 'WHERE parent_id=' . $pid;
        $querySelectDirs = mysqli_query($this->modx->db->conn,$selectDirs);
        if (!$querySelectDirs) {
            $err = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectDirs;
            $this->setError($err);
            return FALSE;
        }

        $mdirs = array();
        while ($l = mysqli_fetch_assoc($querySelectDirs)) {
            $mdirs[$l['cat_name']]['id'] = $l['cat_id']; // goldsky -- to be connected between db <--> fs
            $mdirs[$l['cat_name']]['name'] = $l['cat_name'];
        }
        mysqli_free_result($querySelectDirs);

        // mysqli File list
        $selectFiles = 'SELECT * FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                . 'WHERE dir_id=' . $pid;
        $querySelectFile = mysqli_query($this->modx->db->conn,$selectFiles);
        if (!$querySelectFile) {
            $err = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFiles;
            $this->setError($err);
            return FALSE;
        }

        $mfiles = array();
        while ($l = mysqli_fetch_assoc($querySelectFile)) {
            $mfiles[$l['filename']]['id'] = $l['id'];
            $mfiles[$l['filename']]['name'] = $l['filename'];
        }

        mysqli_free_result($querySelectFile);
            ...
            foreach ($fs as $filePath) {
                ...
                if ($this->validFolder($filePath)) { // as a folder/directory
                    ...
                    if (isset($mdirs[$name])) {
                        ...
                        if ($sync !== TRUE) {
                            $err = __LINE__ . ' : ' . $this->lng['synchro_err'] . ' : ' . $filePath;
                            $this->setError($err);
                            return FALSE;
                        }
                        unset($mdirs[$name]);
                        ...
                        if (!$this->_addAll($filePath, $pid, $userId)) {
                            $err = __LINE__ . ' : ' . $this->lng['all_add_error'] . ' : ' . $filePath;
                            $this->setError($err);
                            return FALSE;
                        }
                    }
                ...
                elseif ($this->validFile($filePath)) {
                    if (isset($mfiles[$name])) {
                        ...
                        $queryUpdateFile = mysqli_query($this->modx->db->conn,$updateFile);
                        if (!$queryUpdateFile) {
                            $err = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateFile;
                            $this->setError($err);
                            return FALSE;
                        }
                        ...
                        if (!$this->_addFile($filePath, $pid, $userId)) {
                            $err = __LINE__ . ' : Unable to INSERT filename into database:' . $filePath;
                            $this->setError($err);
                            return FALSE;
                        }
                    }
        ...
        if (isset($mdirs) && count($mdirs) > 0) {
            ...
            foreach ($mdirs as $key => $value) {
                ...
                if (empty($ids)) {
                    $err = ___LINE__ . ' : ' . $this->lng['err_empty_id'] . ' : ' . $key . '<br />';
                    $this->setError($err);
                    return FALSE;
                }
                ...
                $querySelectFiles = mysqli_query($this->modx->db->conn,$selectFiles);
                if (!$querySelectFiles) {
                    $err = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFiles;
                    $this->setError($err);
                    return FALSE;
                ...
                $fileIds = array();
                while ($l = mysqli_fetch_row($querySelectFiles)) {
                    $fileIds[] = $l[0];
                }
                mysqli_free_result($querySelectFiles);
                ...
                if (count($fileIds) > 0) {
                    ...
                    $queryDeleteFileComments = mysqli_query($this->modx->db->conn,$deleteFileComments);
                    if (!$queryDeleteFileComments) {
                        $err = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deleteFileComments;
                        $this->setError($err);
                        return FALSE;
                    }
                }
                ...
                $queryDeleteFiles = mysqli_query($this->modx->db->conn,$deleteFiles);
                if (!$queryDeleteFiles) {
                    $err = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deleteFiles;
                    $this->setError($err);
                    return FALSE;
                }
            }
    ...
    private function _checkFileDuplication($name, $pid) {
        ...
        $queryCheck = @mysqli_query($this->modx->db->conn,$selectCheck);
        while ($l = @mysqli_fetch_assoc($queryCheck)) {
            $fetchRow[$l['filename']] = $l['filename'];
        }
        mysqli_free_result($queryCheck);
        ...
        $recursiveCheckQuery = @mysqli_query($this->modx->db->conn,$recursiveCheckSelect);
        while ($l = @mysqli_fetch_assoc($recursiveCheckQuery)) {
            $recursiveFetchRow[$l['filename']] = $l['filename'];
        }
        mysqli_free_result($recursiveCheckQuery);
    ...
    private function _dirDropDownOptions($parentId = 0, $selected = 0, $jsActions = NULL) {
        ...
        $querySelectDirs = mysqli_query($this->modx->db->conn,$selectDirs);
        if (!$querySelectDirs) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectDirs;
            return FALSE;
        }

        $numDir = @mysqli_num_rows($querySelectDirs);
        ...
        foreach ($childrenDirs as $childDir) {
            ...
            ;
            $querySelectSubFolders = mysqli_query($this->modx->db->conn,$selectSubFolders);
            if (!$querySelectSubFolders) {
                $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectSubFolders;
                return FALSE;
            }

            $numSub = @mysqli_num_rows($querySelectSubFolders);
            if ($numSub > 0) {
                $output .= $this->_dirDropDownOptions($childDir['cat_id'], $selected, $jsActions);
            }
            mysqli_free_result($querySelectSubFolders);
            //*********************************************************/
        } // foreach ($childrenDirs as $childDir)
        return $output;
    }
    ...
    private function _fileDropDownOptions($parentId = 0, $selected = 0, $jsActions = NULL) {
        ...
        $querySelectFiles = mysqli_query($this->modx->db->conn,$selectFiles);
        if (!$querySelectFiles) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFiles;
            return FALSE;
        }

        $numFile = @mysqli_num_rows($querySelectFiles);
        if ($numFile > 0) {
            $childrenFiles = array();
            $output = '';
            $catThumbId = $this->getDirInfo($this->sanitizedGets['dir_id'], 'cat_thumb_id');
            while ($l = mysqli_fetch_assoc($querySelectFiles)) {
                // DISPLAY
        ...
        $numSub = @mysqli_num_rows($querySelectSubFolders);
        if ($numSub > 0) {
            while ($res = mysqli_fetch_assoc($querySelectSubFolders)) {
    ...
    private function _tagOptions($tag) {
        ...
        $queryDirTags = mysqli_query($this->modx->db->conn,$selectDirTags);

        if (!$queryDirTags) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectDirTags;
            return FALSE;
        }

        $numDirTags = mysqli_num_rows($queryDirTags);

        while ($l = mysqli_fetch_assoc($queryDirTags)) {
        ...
        $queryFileTags = mysqli_query($this->modx->db->conn,$selectFileTags);

        if (!$queryFileTags) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFileTags;
            return FALSE;
        }

        $numFileTags = mysqli_num_rows($queryFileTags);

        while ($l = mysqli_fetch_assoc($queryFileTags)) {
            if ($l['tag'] == '' || $l['tag'] == NULL)
                continue;
            $tagOptions[] = $l['tag'];
        }
    ...
    private function _uploadAll($post, $files) {
        ...
        if (!empty($files['img']['tmp_name'][0])) {
            ...
            for ($i = 0; $i < $countFiles; $i++) {
                ...
                $queryInsertFile = mysqli_query($this->modx->db->conn,$insertFile);
                if (!$queryInsertFile) {
                    $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertFile;
                    continue;
                }
        ...
        if (!empty($post['dir'])) {
            foreach ($post['dir'] as $k => $v) {
                ...
                $selectDirStatus = 'SELECT cat_name, cat_visible FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_dirs '
                        . 'WHERE cat_id=' . $k;
                $querySelectDirStatus = mysqli_query($this->modx->db->conn,$selectDirStatus);
                if (!$querySelectDirStatus) {
                    $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectDirStatus;
                    return FALSE;
                }

                $l = mysqli_fetch_assoc($querySelectDirStatus);
                mysqli_free_result($querySelectDirStatus);
                ...
                $queryUpdateDirStatus = 'UPDATE ' . $this->modx->db->config['table_prefix'] . 'easy2_dirs '
                        ...
                        . 'WHERE cat_id=' . $k;
                $queryUpdateDirStatus = mysqli_query($this->modx->db->conn,$queryUpdateDirStatus);
                if (!$queryUpdateDirStatus) {
                    $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $queryUpdateDirStatus;
                    return FALSE;
                } else {
                    $countRes['ddb'] ++;
                }
        ...
        if (!empty($post['im'])) {
            foreach ($post['im'] as $k => $v) {
                ...
                $selectFileStatus = 'SELECT filename, status FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                        . 'WHERE id=' . $k;
                $querySelectFileStatus = mysqli_query($this->modx->db->conn,$selectFileStatus);
                if (!$querySelectFileStatus) {
                    $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFileStatus;
                    return FALSE;
                }

                $l = mysqli_fetch_assoc($querySelectFileStatus);
                mysqli_free_result($querySelectFileStatus);
                ...
                $updateFileStatus = 'UPDATE ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                        ...
                        . ', modified_by=\'' . $this->modx->getLoginUserID() . '\' '
                        . 'WHERE id=' . $k;
                $queryUpdateFileStatus = mysqli_query($this->modx->db->conn,$updateFileStatus);
                if (!$queryUpdateFileStatus) {
                    $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateFileStatus;
                    return FALSE;
                } else {
                    $countRes['fdb'] ++;
                }
    ...
    private function _unhide($dirId) {
        ...
        $updateDir = 'UPDATE ' . $this->modx->db->config['table_prefix'] . 'easy2_dirs '
                ...
                . ', modified_by=\'' . $this->modx->getLoginUserID() . '\' '
                . 'WHERE cat_id=' . $dirId;
        $queryUpdateDir = mysqli_query($this->modx->db->conn,$updateDir);
        if (!$queryUpdateDir) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateDir;
            return FALSE;
        }
    ...
    private function _unhideFile($fileId) {
        ...
        $queryUpdateFile = mysqli_query($this->modx->db->conn,$updateFile);
        if (!$queryUpdateFile) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateFile;
            return FALSE;
        }
            ...
            foreach ($post['dir'] as $k => $v) {
                ...
                if (is_numeric($k)) {
                    ...
                    $selectFileIds = 'SELECT id FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                            . 'WHERE dir_id IN(' . $implodedDirIds . ')';
                    $querySelectFileIds = mysqli_query($this->modx->db->conn,$selectFileIds);
                    if (!$querySelectFileIds) {
                        $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFileIds;
                        return FALSE;
                    }
                    $fileIds = array();
                    while ($l = mysqli_fetch_row($querySelectFileIds)) {
                        $fileIds[] = $l[0];
                    }
                    mysqli_free_result($querySelectFileIds);

                    $implodedFileIds = implode(',', $fileIds);
                    if (count($fileIds) > 0) {
                        $delComments = 'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_comments '
                                . 'WHERE file_id IN(' . $implodedFileIds . ')';
                        $delCommentsQuery = mysqli_query($this->modx->db->conn,$delComments);
                        if (!$delCommentsQuery) {
                            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $delComments;
                            return FALSE;
                        }
                    }
                    $delFiles = 'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                            . 'WHERE dir_id IN(' . $implodedDirIds . ')';
                    $delFilesQuery = mysqli_query($this->modx->db->conn,$delFiles);
                    if (!$delFilesQuery) {
                        $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $delFiles;
                        return FALSE;
                    }
                    $res['fdb'][0] += mysqli_affected_rows();
                    if (count($ids) > 0) {
                        $res['ddb'][0] += count($ids);
                    } else {
                        $res['ddb'][1] ++;
        ...
        if (!empty($post['im'])) {
            foreach ($post['im'] as $k => $v) {
                ...
                if (is_numeric($k)) {
                    ...
                    $querySelectFileId = mysqli_query($this->modx->db->conn,$selectFileId);
                    if (!$querySelectFileId) {
                        $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFileId;
                        return FALSE;
                    }

                    $fileIds = array();
                    while ($l = mysqli_fetch_row($querySelectFileId))
                        $fileIds[] = $l[0];
                    mysqli_free_result($querySelectFileId);

                    if (count($fileIds) > 0) {
                        $deleteComments = 'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_comments '
                                . 'WHERE file_id IN(' . implode(',', $fileIds) . ')';
                        $queryDeleteComments = mysqli_query($this->modx->db->conn,$deleteComments);
                        if (!$queryDeleteComments) {
                            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deleteComments;
                            return FALSE;
                        }
                    }
                    $deleteFile = 'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_files WHERE id=' . (int) $k;
                    $queryDeleteFile = mysqli_query($this->modx->db->conn,$deleteFile);
                    if (!$queryDeleteFile) {
                        $res['fdb'][1] ++;
                        $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deleteFile;
                        return FALSE;
                    } else {
                        $res['fdb'][0] ++;
                    }
                ...
                if (!empty($v)) {
                    ...
                    if (is_file($newFile['decoded'])) {
                        ...
                        if ($moveFile) {
                            ...
                            if (is_numeric($k)) {
                                $files = array();
                                $filesRes = mysqli_query($this->modx->db->conn,
                                        'SELECT id, dir_id '
                                        . 'FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                                        . 'WHERE id=' . (int) $k
                                );
                                while ($l = mysqli_fetch_assoc($filesRes)) {
                                    $files[$l['id']]['dir_id'] = $l['dir_id'];
                                }
                                mysqli_free_result($filesRes);
                                ...
                                ;
                                $filesCheckQuery = mysqli_query($this->modx->db->conn,$filesCheckSelect);
                                while ($f = mysqli_fetch_assoc($filesCheckQuery)) {
                                    $filesCheck[$f['ai']]['filename'] = $f['bf'];
                                }
                                mysqli_free_result($filesCheckQuery);
                                ...
                                if (mysqli_query($this->modx->db->conn,$updateFile)) {
                                    $res['fdb'][0] ++;
                                } else {
                                    $res['fdb'][1] ++;
                                }
    ...
    private function _deleteDir($get) {
        ...
        if (is_numeric($get['dir_id'])) {
            ...
            $fileIds = array();
            $res = mysqli_query($this->modx->db->conn,
            ...
            if (count($fileIds) > 0) {
                mysqli_query($this->modx->db->conn,
                        'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_comments '
                        . 'WHERE file_id IN(' . @implode(',', $fileIds) . ')');
            }
            mysqli_query($this->modx->db->conn,'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                    . 'WHERE dir_id IN(' . @implode(',', $ids) . ')');
        }
    ...
    private function _deleteFile($get) {
        ...
        if (is_numeric($get['file_id'])) {
            ...
            if (!$queryDeleteFile) {
                ...
                mysqli_query($this->modx->db->conn,'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_comments WHERE file_id=' . $get['file_id']);
                $res['db'] ++;
            }
        }
    ...
    public function saveE2gSettings($entries, $buildDefault = FALSE) {
        ...
        if ($buildDefault) {
            ...
            $queryDeleteConfigs = mysqli_query($this->modx->db->conn,$deleteConfigs);
            if (!$queryDeleteConfigs) {
                $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deleteConfigs;
                return FALSE;
            }
            // else
            foreach ($entries as $k => $v) {
                $insertConfigs = 'INSERT INTO ' . $this->modx->db->config['table_prefix'] . 'easy2_configs '
                        . 'SET `cfg_key`=\'' . $k . '\', `cfg_val`=\'' . mysqli_real_escape_string($this->modx->db->conn,$v) . '\'';
                $queryInsertConfigs = mysqli_query($this->modx->db->conn,$insertConfigs);
                if (!$queryInsertConfigs) {
                    $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertConfigs;
                    return FALSE;
                }
            }
    ...
    private function _ignoreIp($get) {
        ...
        $queryInsertIgnoredIp = mysqli_query($this->modx->db->conn,$insertIgnoredIp);
        if (!$queryInsertIgnoredIp) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertIgnoredIp;
            return FALSE;
        }
        $updateCommentStatus = 'UPDATE ' . $this->modx->db->config['table_prefix'] . 'easy2_comments '
                . 'SET STATUS=\'0\' WHERE ip_address=\'' . $get['ip'] . '\'';
        $queryUpdateCommentStatus = mysqli_query($this->modx->db->conn,$updateCommentStatus);
        if (!$queryUpdateCommentStatus) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateCommentStatus;
            return FALSE;
        }
    ...
    private function _unignoreIp($get) {
        ...
        $queryDeleteIgnoredIp = mysqli_query($this->modx->db->conn,$deleteIgnoredIp);
        if (!$queryDeleteIgnoredIp) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deleteIgnoredIp;
            return FALSE;
        }
        $updateCommentStatus = 'UPDATE ' . $this->modx->db->config['table_prefix'] . 'easy2_comments '
            ...
            foreach ($post['dir'] as $k => $v) {
                if (!empty($v)) {
                    ...
                    $selectDirTags = 'SELECT cat_tag FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_dirs '
                            . 'WHERE cat_id=' . $k;
                    $querySelectDirTags = mysqli_query($this->modx->db->conn,$selectDirTags);

                    if (!$querySelectDirTags) {
                        $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectDirTags;
                        return FALSE;
                    }
                    while ($l = mysqli_fetch_assoc($querySelectDirTags)) {
                        $dirTags = $l['cat_tag'];
                    }
                    mysqli_free_result($querySelectDirTags);
                    ...
                    if (count($newTags) > 0) {
                        ...
                        $updateDirTags = 'UPDATE ' . $this->modx->db->config['table_prefix'] . 'easy2_dirs '
                                ...
                                . ', last_modified=NOW() '
                                . 'WHERE cat_id=' . $k;
                        $queryUpdateDirTags = mysqli_query($this->modx->db->conn,$updateDirTags);
                        if (!$queryUpdateDirTags) {
                            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateDirTags;
                            return FALSE;
                        }
                    }
                } // if (!empty($v))
            ...
            foreach ($post['im'] as $k => $v) {
                if (!empty($v)) {
                    ...
                    $selectFileTags = 'SELECT tag FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                            . 'WHERE id=' . $k;
                    $querySelectFileTags = mysqli_query($this->modx->db->conn,$selectFileTags);
                    if (!$querySelectFileTags) {
                        $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFileTags;
                        return FALSE;
                    }
                    while ($l = mysqli_fetch_assoc($querySelectFileTags)) {
                        $fileTags = $l['tag'];
                    }
                    mysqli_free_result($querySelectFileTags);
                    ...
                    if (count($newTags) > 0) {
                        ...
                        $updateFileTags = 'UPDATE ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                                ...
                                . 'WHERE id=' . $k
                        ;
                        $queryUpdateFileTags = mysqli_query($this->modx->db->conn,$updateFileTags);
                        if (!$queryUpdateFileTags) {
                            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateFileTags;
                            return FALSE;
                        }
                    }
                } // if (!empty($v))
    ...
    private function _commentListActions($post) {
        ...
        if ($post['listCommentActions'] == 'delete') {
            ...
            foreach ($post['comments'] as $comId) {
                ...
                $queryUpdateComment = mysqli_query($this->modx->db->conn,$updateComment);
                if (!$queryUpdateComment) {
                    $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateComment;
                    return FALSE;
                }
            }
        }
    ...
    private function _commentDelete($id) {
        $selectFileId = 'SELECT file_id FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_comments '
                . 'WHERE id=' . (int) $id;
        $fileId = mysqli_result(mysqli_query($this->modx->db->conn,$selectFileId), 0, 0);
        if (!$fileId) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFileId;
            return FALSE;
        }
        ...
        $deleteComment = 'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_comments '
                . 'WHERE id=' . (int) $id;
        $queryDeleteComment = mysqli_query($this->modx->db->conn,$deleteComment);
        if (!$queryDeleteComment) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deleteComment;
            return FALSE;
        } else {
            $updateFileComment = 'UPDATE ' . $this->modx->db->config['table_prefix'] . 'easy2_files '
                    ...
                    . ', modified_by=\'' . $this->modx->getLoginUserID() . '\' '
                    . 'WHERE id=' . $fileId;
            $queryUpdateFileComment = mysqli_query($this->modx->db->conn,$updateFileComment);
            if (!$queryUpdateFileComment) {
                $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateFileComment;
                return FALSE;
            }
        }
        return TRUE;
    ...
    private function _commentUnhide($id) {
        $updateComment = 'UPDATE ' . $this->modx->db->config['table_prefix'] . 'easy2_comments '
                . 'SET status=\'1\' '
                . 'WHERE id=' . $id;
        $query = mysqli_query($this->modx->db->conn,$updateComment);
        $queryUpdateComment = mysqli_query($this->modx->db->conn,$updateComment);
        if (!$queryUpdateComment) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateComment;
            return FALSE;
    ...
    private function _savePlugin($post) {
        ...
        $queryInsertPlugin = mysqli_query($this->modx->db->conn,$insertPlugin);
        if (!$queryInsertPlugin) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertPlugin;
            return FALSE;
        }

        $pluginId = mysqli_insert_id();
        foreach ($eventsArray as $evtId) {
            $insertPluginEvt = 'INSERT INTO ' . $this->modx->db->config['table_prefix'] . 'easy2_plugin_events '
                    ...
                    . ', evtid=\'' . $evtId . '\'';
            $queryInsertPluginEvt = mysqli_query($this->modx->db->conn,$insertPluginEvt);
            if (!$queryInsertPluginEvt) {
                $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertPluginEvt;
                return FALSE;
            }
        }
    ...
    private function _deletePlugin($get) {
        $delete = mysqli_query($this->modx->db->conn,'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_plugins '
                . 'WHERE id="' . $get['plugin_id'] . '"'
        );
        if ($delete) {
            mysqli_query($this->modx->db->conn,'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_plugin_events '
    ...
    private function _updatePlugin($post) {
        ...
        $queryUpdatePlugin = mysqli_query($this->modx->db->conn,$updatePlugin);
        if (!$queryUpdatePlugin) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updatePlugin;
            return FALSE;
        }

        $deletePluginEvents = 'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_plugin_events '
                . 'WHERE pluginid=\'' . $pluginId . '\'';
        $queryDeletePluginEvents = mysqli_query($this->modx->db->conn,$deletePluginEvents);
        if (!$queryDeletePluginEvents) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deletePluginEvents;
            return FALSE;
        }
        ...
        foreach ($eventsArray as $evtId) {
            $insertPluginEvents = 'INSERT INTO ' . $this->modx->db->config['table_prefix'] . 'easy2_plugin_events '
                    . 'SET pluginid=\'' . $pluginId . '\' '
                    . ', evtid=\'' . $evtId . '\'';
            $queryInsertPluginEvents = mysqli_query($this->modx->db->conn,$insertPluginEvents);
            if (!$queryInsertPluginEvents) {
                $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertPluginEvents;
                return FALSE;
            }
        }
    ...
    private function _saveViewer($post) {
        ...
        ;
        $queryInservtViewer = mysqli_query($this->modx->db->conn,$insertViewer);
        if (!$queryInservtViewer) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertViewer;
            return FALSE;
        }
    ...
    private function _updateViewer($post) {
        ...
        ;
        $updateQuery = mysqli_query($this->modx->db->conn,$updateViewer);
        if (!$updateQuery) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateViewer;
            return FALSE;
        }
    ...
    private function _deleteViewer($get) {
        mysqli_query($this->modx->db->conn,'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_viewers '
    ...
    private function _saveSlideshow($post) {
        ...
        if (is_array($post['name'])) {
            ...
            for ($i = 0; $i < $countPost; $i++) {
                ...
                ;
                $queryInsertSlideshow = mysqli_query($this->modx->db->conn,$insertSlideshow);
                if (!$queryInsertSlideshow) {
                    $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertSlideshow;
                    return FALSE;
                }
            }
        } else {
    ...
    private function _deleteSlideshow($get) {
        mysqli_query($this->modx->db->conn,'DELETE FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_slideshows '
                . 'WHERE id="' . $get['ssid'] . '"'
        );
    }
    ...
    private function _updateSlideshow($post) {
        ...
        ;
        $queryUpdateSlideshow = mysqli_query($this->modx->db->conn,$updateSlideshow);
        if (!$queryUpdateSlideshow) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateSlideshow;
            return FALSE;
        }
    ...
    private function _editFile($gdir, $post) {
        ...
        if (!$queryUpdateFile) {
            $_SESSION['easy2err'][] = __LINE__ . ' : ' . $this->lng['update_err'];
            $_SESSION['easy2err'][] = __LINE__ . ' : ' . mysqli_error($this->modx->db->conn);
    ...
    private function _synchroUserGroups() {
        ...
        for ($i = 0; $i < $countModxMemberGroups; $i++) {
            ...
            $queryInsertMgrUser = mysqli_query($this->modx->db->conn,$insertMgrUser);
        ...
        if (!empty($e2gMgrGroupIds)) {
            foreach ($e2gMgrGroupIds as $id) {
                ...
                $queryDeleteMgrUser = mysqli_query($this->modx->db->conn,$deleteMgrUser);
    ...
    private function _saveWebGroupsAccess($webGroupIds, $type, $id) {
        ...
        $queryDeleteWebAccess = mysqli_query($this->modx->db->conn,$deleteWebAccess);
        if (!$queryDeleteWebAccess) {
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deleteWebAccess;
            return FALSE;
        }
        ...
        foreach ($webGroupIds as $webGroupId) {
            ...
            $queryInsertWebAccess = mysqli_query($this->modx->db->conn,$insertWebAccess);
            if (!$queryInsertWebAccess) {
                $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertWebAccess;
                return FALSE;
            }
        }
        return TRUE;
    ...
    private function _saveMgrAccess($post) {
        ...
        $queryUpdateMgrUser = mysqli_query($this->modx->db->conn,$updateMgrUser);
        if (!$queryUpdateMgrUser) {
            $_SESSION['easy2err'][] = __LINE__ . ' : ' . $this->lng['user_update_err'];
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $updateMgrUser;
            return FALSE;
        }
    ...
    private function _saveDirWebAccess($post) {
        ...
        $queryDeleteDirWebAccess = mysqli_query($this->modx->db->conn,$deleteDirWebAccess);
        if (!$queryDeleteDirWebAccess) {
            $_SESSION['easy2err'][] = __LINE__ . ' : ' . $this->lng['user_update_err'];
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deleteDirWebAccess;
            return FALSE;
        }
        ...
        foreach ($dirWebAccess as $v) {
            ...
            $queryInsertDirWebAccess = mysqli_query($this->modx->db->conn,$insertDirWebAccess);
            if (!$queryInsertDirWebAccess) {
                $_SESSION['easy2err'][] = __LINE__ . ' : ' . $this->lng['user_update_err'];
                $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertDirWebAccess;
                return FALSE;
            }
        }
        $_SESSION['easy2suc'][] = __LINE__ . ' : ' . $this->lng['user_update_suc'];
    ...
    private function _saveFileWebAccess($post) {
        ...
        $queryDeleteFileWebAccess = mysqli_query($this->modx->db->conn,$deleteFileWebAccess);
        if (!$queryDeleteFileWebAccess) {
            $_SESSION['easy2err'][] = __LINE__ . ' : ' . $this->lng['user_update_err'];
            $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $deleteFileWebAccess;
            return FALSE;
        }
        ...
        foreach ($fileWebAccess as $v) {
            ...
            $queryInsertFileWebAccess = mysqli_query($this->modx->db->conn,$insertFileWebAccess);
            if (!$queryInsertFileWebAccess) {
                $_SESSION['easy2err'][] = __LINE__ . ' : ' . $this->lng['user_update_err'];
                $_SESSION['easy2err'][] = __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $insertFileWebAccess;
                return FALSE;
            }
        }
        $_SESSION['easy2suc'][] = __LINE__ . ' : ' . $this->lng['user_update_suc'];
...
//    }
    ...
    private function _checkConfigCompletion() {
        ...
        // delete the config file, because this will always be checked as an upgrade option
        if (file_exists(realpath(E2G_MODULE_PATH . 'includes/configs/config.easy2gallery.php')) && $this->sanitizedGets['e2gpg'] != $this->e2gModCfg['e2gPages']['config']['e2gpg']
        ) {
    ...
    public function actionIcon($buttonName, $getParams = array(), $attributes = NULL, $index = NULL) {
        ...
        if ($buttonName == 'hide_dir') {
            $button .= '
                    <i class="icon-custom bordered-2x icon-bg-sea fa fa-eye fa-2x icon-vs" title="' . $this->lng['visible'] . '"></i>';
        } elseif ($buttonName == 'unhide_dir') {
            $button .= '
                    <i class="icon-custom bordered-2x icon-bg-aqua fa fa-eye-slash icon-vs" title="' . $this->lng['hidden'] . '"></i>';
        } elseif ($buttonName == 'edit_dir') {
            $button .= '
                    <i class="icon-custom bordered-2x icon-bg-orange fa fa-pencil icon-vs" title="' . $this->lng['edit'] . '"></i>';
        } elseif ($buttonName == 'add_dir') {
            $button .= '
                    <i class="icon-custom icon-no-border icon-bg-green fa fa-plus-circle fa-2x icon-vs" title="' . $this->lng['add_to_db'] . '"></i>';
        } elseif ($buttonName == 'delete_dir') {
            $button .= '
                    <i class="icon-custom bordered-2x icon-bg-red fa fa-trash icon-vs" title="' . $this->lng['delete'] . '"></i>';
        }

        // images
        if ($buttonName == 'hide_file') {
            $button .= '
                    <i class="icon-custom bordered-2x icon-bg-sea fa fa-eye icon-vs" title="' . $this->lng['visible'] . '"></i>';
        } elseif ($buttonName == 'unhide_file') {
            $button .= '
                    <i class="icon-custom bordered-2x icon-bg-aqua fa fa-eye-slash icon-vs" title="' . $this->lng['hidden'] . '"></i>';
        } elseif ($buttonName == 'comments') {
            $button .= '
                    <i class="icon-custom bordered-2x icon-bg-blue fa fa-comments icon-vs" title="' . $this->lng['comments'] . '"></i>';
    ...
    public function loadSettings() {
        ...
        if (!empty($configTablesValues)) {
            ...
            if ($configsQuery) {
                while ($row = mysqli_fetch_assoc($configsQuery)) {
        ...
        if ($saveE2gSettings && $_SESSION['installE2g'] === FALSE // exclude new installation
        ) {
...
//                exit;
    ...
    public function setError($msg) {
        $this->error = $msg;
        $_SESSION['easy2err'][] = $msg;
        $this->modx->logEvent(0, 3, $msg, 'Easy 2 Gallery');
    }
