...
//echo "<p>modified selected files: ". $selectFiles."</p>";
        ...
        if (!$querySelectFiles) {
            echo __LINE__ . ' : damn#' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFiles . '<br />';
            return FALSE;
        }

        $fileNumRows = mysqli_num_rows($querySelectFiles);
        if ($fileNumRows === 0) {
            return;
        }
        $fileThumbs = array();
    ...
    private function _countAllDirs() {
        ...
        $querySelectDirCount = mysqli_query($this->modx->db->conn,$selectDirCount);
        if (!$querySelectDirCount) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectDirCount . '<br />';
            return FALSE;
        }

        //$resultCountDirs = mysqli_result($querySelectDirCount, 0, 0);
        $resultCountDirs = mysqli_data_seek($querySelectDirCount, 0);

        mysqli_free_result($querySelectDirCount);
...
//            unset($crumbsPathArray[1]);
    ...
    private function _dirThumbs() {
        ...
        if (!$querySelectDirs) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectDirs . '<br />';
            return FALSE;
        }
        $this->_dirNumRows += mysqli_num_rows($querySelectDirs);
        ...
        while ($l = mysqli_fetch_assoc($querySelectDirs)) {
            ...
            if ($this->e2gSnipCfg['use_redirect_link'] === TRUE && !empty($l['cat_redirect_link'])) {
                ...
                $l['link'] = $this->modx->makeUrl(
                                 $this->modx->documentIdentifier  //$this->modx->documentObject['id']
                                , $this->modx->aliases
                                , 'sid=' . $this->e2gSnipCfg['e2g_static_instances'])
                        . '&amp;gid=' . $l['cat_id']
                        . (isset($this->e2gSnipCfg['static_tag']) ? '&amp;tag=' . $this->e2gSnipCfg['static_tag'] : '')
...
//echo '<p>line: '.__LINE__.'<br> gallery link: '.$l['link'].'</p>';
            /**
    ...
    public function imgFile() {
        ...
        $querySelectFiles = mysqli_query($this->modx->db->conn,$selectFiles);
        if (!$querySelectFiles) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFiles . '<br />';
            return FALSE;
        }
        $fileNumRows = mysqli_num_rows($querySelectFiles);
        if ($fileNumRows === 0) {
    ...
    private function _loadThumbPlaceholders($row) {
        ...
        if (intval($this->e2gSnipCfg['ecm']) === 1 && !$checkIgnoredIp) {
            ...
            if (isset($glibs[$this->e2gSnipCfg['glib']])) {
                ...
                // added &p=comm to comments link
                $row['comments'] = '<a href="' . $_SESSION['e2g_view_processor_link'] .'rid=' . $row['id'] . '&p=com" ' . $glibs[$this->e2gSnipCfg['glib']]['clibact'] . '>' . $row['comments'] . '</a>';
    ...
    public function slideshow($slideshow) {
        ...
        if (isset($this->_sanitizedGets['fid'])
            ...
            // include the available slideshow from database
                ...
                $queryIndexFile = mysqli_query($this->modx->db->conn,$selectIndexFile);
                if (!$queryIndexFile) {
                    echo __LINE__ . ' : ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectIndexFile . '<br />';
                    return FALSE;
                }
                $row = mysqli_fetch_row($queryIndexFile);
                $dbIndexFile = $row[0];
    ...
    private function _getSlideShowFiles() {
        ...
        if (!empty($this->e2gSnipCfg['gid']) && $this->modx->documentIdentifier !== intval($this->e2gSnipCfg['landingpage'])) {
            ...
            $querySelectFiles = mysqli_query($this->modx->db->conn,$selectFiles);
            if (!$querySelectFiles) {
                echo __LINE__ . ' : ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectFiles . '<br />';
                return FALSE;
            }

            while ($row = mysqli_fetch_assoc($querySelectFiles)) {
    ...
    public function landingPage($fileId) {
        ...
        if (!$query) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $select . '<br />';
            return FALSE;
        }
...
//                if (strpos($_SERVER['DOCUMENT_ROOT'], '/') === (int) 0) {
    ...
    private function _comments($fileId) {
                ...
                if ($this->e2gSnipCfg['recaptcha'] === '1' && $_POST['recaptcha_response_field']) {
                    ...
                    if ($_POST["recaptcha_response_field"]) {
                        ...
                        if (!$resp->is_valid) {
                            ...
                            if (mysqli_query($this->modx->db->conn,$comInsert)) {
                                mysqli_query($this->modx->db->conn,'UPDATE ' . $this->modx->db->config['table_prefix'] . 'easy2_files SET comments=comments+1 WHERE id=' . $fileId);
        ...
        $querySelectComments = mysqli_query($this->modx->db->conn,$selectComments);
        if (!$querySelectComments) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectComments . '<br />';
            return FALSE;
        }

        $rowClassNum = 0;
        while ($l = mysqli_fetch_assoc($querySelectComments)) {
            $l['i'] = $rowClassNum % 2;
            ...
            $_P['comment_body'] .= $this->filler($this->getTpl('comments_row_tpl'), $l);
            $rowClassNum++;
        }
        mysqli_free_result($querySelectComments);

        $_P['pages_permalink'] = '<a href="#" name="lpcmtpg' . $cpn . '"></a>';

        // Comment pages
        // added  AND STATUS=1
        $selectCountComments = 'SELECT COUNT(*) FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_comments WHERE file_id = ' . $fileId .' AND STATUS=1';
        $querySelectCountComments = mysqli_query($this->modx->db->conn,$selectCountComments);
        if (!$querySelectCountComments) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectCountComments . '<br />';
            return FALSE;
        }

        list($cnt) = mysqli_fetch_row($querySelectCountComments);
        mysqli_free_result($querySelectCountComments);
...
//                return $this->modx->sendErrorPage();
    ...
    private function _checkTaggedDirIds($tag, $id=1) {
        ...
        $querySelectTaggedDirs = mysqli_query($this->modx->db->conn,$selectTaggedDirs);
        if (!$querySelectTaggedDirs) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectTaggedDirs . '<br />';
            return FALSE;
        }

        $taggedDirs = array();
        while ($l = mysqli_fetch_assoc($querySelectTaggedDirs)) {
            $taggedDirs[] = $l['cat_id'];
        }

        mysqli_free_result($querySelectTaggedDirs);
    ...
    private function _checkTaggedFileIds($tag, $id) {
        ...
        $querySelectTaggedFiles = mysqli_query($this->modx->db->conn,$selectTaggedFiles);
        if (!$querySelectTaggedFiles) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectTaggedFiles . '<br />';
            return FALSE;
        }

        $taggedFiles = array();
        while ($l = mysqli_fetch_assoc($querySelectTaggedFiles)) {
            $taggedFiles[$l['id']] = $l['id'];
        }
        mysqli_free_result($querySelectTaggedFiles);
    ...
    private function _getParentInfo($dynamicId, $field = 'cat_id', $trigger=NULL) {
        ...
        $queryParent = mysqli_query($this->modx->db->conn,$selectParent);
        if (!$queryParent) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectParent . '<br />';
            return FALSE;
        }

        $parent = array();
        while ($row = mysqli_fetch_assoc($queryParent)) {
    ...
    private function _getSiblingInfo($trigger, $dynamicId, $field, $siblingCounter) {
        ...
        $queryChildren = mysqli_query($this->modx->db->conn,$selectChildren);
        if (!$queryChildren) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectChildren . '<br />';
            return FALSE;
        }

        $siblings = array();
        while ($row = mysqli_fetch_assoc($queryChildren)) {
