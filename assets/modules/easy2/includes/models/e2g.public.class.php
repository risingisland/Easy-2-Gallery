...
//            $convertedText = UTF8::convert_from($text, $mbDetectEncoding);
    ...
    public function sanitizedString($string, $chars = array('/', "'", '"', '(', ')', ';', '>', '<'), $allowedTags = array(), $preg=FALSE) {
        ...
        //$string = mysql_real_escape_string($string);
         $string = $this->modx->db->escape($string);
        return $string;
    }

    /** "'
     * To get directorys information
     * @param   int     $dirId  galler ID
     * @param   string  $field  database field
     * @return  string  the directorys data
     */
    public function getDirInfo($dirId, $field) {
        ...
        if (!($res = mysqli_query($this->modx->db->conn,$q))) {
...
//        $encodingHtml= htmlspecialchars($endPath, ENT_QUOTES);
    ...
    public function plugin($e2gEvtName, $e2gEvtParams = array(), $e2gPluginName = NULL, $respectDisabling = TRUE) {
        ...
        if (!$queryIndexFile) {
            echo __METHOD__ . ', ' . __LINE__ . ' : ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectIndexFile;
            return FALSE;
        } else {
            while ($row = mysqli_fetch_assoc($queryIndexFile)) {
    ...
    public function folderImg($gid, $gdir) {
        ...
        $queryDbFile = mysqli_query($this->modx->db->conn,$selectDbFile);
        if (!$queryDbFile) {
            $o = __LINE__ . ': __METHOD__ = ' . __METHOD__ . '<br />';
            $o .= mysqli_error($this->modx->db->conn) . '<br />' . $selectDbFile . '<br />';
            echo $o;
            return FALSE;
        }

        while ($l = mysqli_fetch_assoc($queryDbFile)) {
        ...
        if (!empty($catThumbId)) {
            ...
            if (file_exists(realpath($this->e2gDecode($gdir . $catThumbPath . $catThumbName)))) {
                $selectThumbFile = 'SELECT * '
                        ...
                        . 'WHERE id = ' . $catThumbId
                ;
                $queryThumbFile = mysqli_query($this->modx->db->conn,$selectThumbFile);
                if (!$queryThumbFile) {
                    $o = __LINE__ . ': __METHOD__ = ' . __METHOD__ . '<br />';
                    $o .= mysqli_error($this->modx->db->conn) . '<br />' . $selectThumbFile . '<br />';
                    echo $o;
                    return FALSE;
                }

                $specifiedFolderImg = mysqli_fetch_assoc($queryThumbFile);
                mysqli_free_result($queryThumbFile);
            }
        }
...
//                . 'AND A.cat_visible = 1'         // disabled, because Module needs to see them ALL
        ...
        $queryFiles = mysqli_query($this->modx->db->conn,$selectFiles);
        if (!$queryFiles) {
            $o = __LINE__ . ': __METHOD__ = ' . __METHOD__ . '<br />';
            $o .= mysqli_error($this->modx->db->conn) . '<br />' . $selectFiles . '<br />';
            echo $o;
            return FALSE;
        }

        while ($l = mysqli_fetch_assoc($queryFiles)) {
            $files[] = $l;
        }
        mysqli_free_result($queryFiles);
    ...
    public function getPath($dirId, $option='cat_name', $format='string') {
       // added to check for installing, because if the tables are not installed "table does not exist errors"
        ...
        $queryDir = mysqli_query($this->modx->db->conn,$selectDir);
                /*$queryDir = $this->modx->db->select('A.parent_id, A.cat_id,A.cat_name,A.cat_alias ',
                                            $this->modx->db->config['table_prefix'] . 'easy2_dirs A, '. $this->modx->db->config['table_prefix'] . 'easy2_dirs B ',
                                            'B.cat_id=' . $dirId . ' '. 'AND B.cat_left BETWEEN A.cat_left AND A.cat_right ',
                                            'A.cat_left ASC ',
                                                                                        '');
        */
           if (!$queryDir) {
    ...
    public function checkIgnoredIp() {
        ...
        $resultCountIgnIps = $this->modx->db->getValue(
                                $this->modx->db->select(
                                'COUNT(ign_ip_address)', $this->modx->getFullTableName( 'easy2_ignoredip' ), 'ign_ip_address="'.$ip.'"', '', '' ) );
        ...
        $querySelectCountIgnIp = mysqli_query($this->modx->db->conn,$selectCountIgnIps);
        if (!$querySelectCountIgnIp) {
            echo __LINE__ . ' : #' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $selectCountIgnIps . '<br />';
            return FALSE;
        }
        $resultCountIgnIps = mysqli_data_seek($querySelectCountIgnIp, 0);
        mysqli_free_result($querySelectCountIgnIp);
        */
