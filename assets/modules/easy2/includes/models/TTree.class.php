...
global $modx;
        ...
        $query = 'SELECT * FROM ' . $this->table . ' WHERE '
                . $this->id . ' = ' . $parentId;
        $result = mysqli_query($modx->db->conn,$query);
        if (mysqli_error($modx->db->conn)) {
            $this->error = '#' . mysqli_errno($modx->db->conn) . ' ' . mysqli_error($modx->db->conn);
            return false;
        ...
        if (!mysqli_query($modx->db->conn,$query)) {
            $this->error = '#' . mysqli_errno($modx->db->conn) . ' ' . mysqli_error($modx->db->conn);
            return false;
        }
        ...
        if (mysqli_query($modx->db->conn,$query)) {
            ...
            return mysqli_insert_id( $modx->db->conn );
        } else {
            $this->error = '#' . mysqli_errno($modx->db->conn) . ' ' . mysqli_error($modx->db->conn);
            return false;
        }
...
global $modx;
        ...
        if (!$c) {
            $select_c = 'SELECT * FROM ' . $this->table . ' WHERE '
                    . $this->id . ' = ' . $id;
            $result_select_c = mysqli_query($modx->db->conn,$select_c);

            // set the old parent's handler variable
            $c = mysqli_fetch_assoc($result_select_c);
        }
        ...
        $select_to = 'SELECT * FROM ' . $this->table                                     // goldsky
                . ' WHERE ' . $this->id . ' = ' . $to_id;
        if (!($result_select_to = mysqli_query($modx->db->conn,$select_to))) {
            $this->error = __LINE__ . ' : #' . mysqli_errno($modx->db->conn) . '<br />';
            $this->error .= __LINE__ . ' : ' . mysqli_error($modx->db->conn) . '<br />';
            $this->error .= __LINE__ . ' : ' . $select_to;
            return false;
        }
        // set the new parent's handler variable
        $to = mysqli_fetch_assoc($result_select_to);
        ...
        if (!mysqli_query($modx->db->conn,$update_between_c)) {
            $this->error = __LINE__ . ' : #' . mysqli_errno($modx->db->conn) . '<br />';
            $this->error .= __LINE__ . ' : ' . mysqli_error($modx->db->conn) . '<br />';
...
//        $query = 'UPDATE ' . $this->table . ' SET '
        ...
        if (mysqli_query($modx->db->conn,$update_final)) {
    ...
    function reindex() {
        ...
        if (!mysqli_query($modx->db->conn,$update)) {
            $this->error = '#' . mysqli_errno($modx->db->conn) . ' ' . mysqli_error($modx->db->conn);
            return false;
        }
    }
    ...
    function clear() {
global $modx;
        if (!mysqli_query($modx->db->conn,'DELETE FROM ' . $this->table)) {
            $this->error = '#' . mysqli_errno($modx->db->conn) . ' ' . mysqli_error($modx->db->conn);
            return false;
        }
    ...
    function sql2array($sql, $keyField='') {
        global $modx;        
        if (empty($sql) || !($query = mysqli_query($modx->db->conn,$sql))) {
            return false;
        }
