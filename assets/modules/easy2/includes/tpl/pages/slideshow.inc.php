    ...
    ?>
        ...
        <form action="<?php echo $this->e2gModCfg['index']; ?>&amp;act=save_slideshow" method="post">
            <ul>
                ...
                $query_slideshows = mysqli_query($this->modx->db->conn,$select_slideshows);
                if (!$query_slideshows)
                    die(__LINE__ . ': ' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $select_slideshows);
                else {
                    $numrow_slideshows = mysqli_num_rows($query_slideshows);
                    while ($row = mysqli_fetch_assoc($query_slideshows)) {
                // added here so that empty rows don't show
                              if($row['name'] !=''){
					?>
                    <li>
                            <a href="<?php echo $this->e2gModCfg['index'] . '&amp;page=duplicate_slideshow&amp;ssid=' . $row['id']; ?>"
                               ...
                               title="<?php echo $this->lng['copy']; ?>">
                                <i class="icon-custom icon-no-border icon-bg-orange fa fa-files-o fa-2x icon-vs"></i>
                            </a>
