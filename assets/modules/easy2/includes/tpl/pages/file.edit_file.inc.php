<?php
	...
	$res = mysqli_query($this->modx->db->conn,'SELECT * FROM ' . $this->modx->db->config['table_prefix'] . 'easy2_files WHERE id=' . (int) $this->sanitizedGets['file_id']);
	...
	<script type="text/javascript">
		...
		<div class="tab-page" id="tabEditFilePage">
			...
			<table border="0" cellspacing="0" cellpadding="2" class="aForm" width="100%">
				<tr>
					<td style="vertical-align: top; width: 25%;" nowrap="nowrap">
						<table>
							<tr>
								<td>
									<a href="<?php echo MODX_SITE_URL . $this->e2gModCfg['gdir'] . $row['filename']; ?>" class="highslide" onclick="return hs.expand(this)">
										<img src='<?php echo E2G_MODULE_URL; ?>preview.easy2gallery.php?path=<?php echo $this->e2gModCfg['gdir'] . $row['filename']; ?>' alt="" />
									</a>
									<hr />
									<ul>
										<li>
											<b><?php echo $this->lng['date']; ?> :</b> <?php echo $this->getTime($row['date_added'], $row['last_modified'], $this->e2gModCfg['gdir'] . $row['filename']); ?>
										</li>
										<li>
											<b><?php echo $this->lng['size']; ?> :</b> <?php echo round($row['size'] / 1024); ?> Kb
										</li>
										<li>
											<b><?php echo $this->lng['w']; ?> :</b> <?php echo $row['width']; ?> px</li>
										<li>
											<b><?php echo $this->lng['h']; ?> :</b> <?php echo $row['height']; ?> px
										</li>
									</ul>
								</td>
							</tr>
					...
					<td>
						<form name="list" action="<?php echo $this->e2gModCfg['index'] . '&amp;act=save_file&amp;pid=' . $this->e2gModCfg['parent_id'] . (isset($this->sanitizedGets['tag']) ? '&amp;tag=' . $this->sanitizedGets['tag'] : NULL); ?>" method="post">
							...
							<table id="file_edit" width="100%">
								...
								<tr>
									<td><b><?php echo $this->lng['file_rename']; ?></b></td>
									<td valign="top"><b>:</b></td>
									<td>
										<span><input name="newfilename" type="text" value="<?php echo $filename; ?>" size="20" style="text-align:right; max-width:90%" /> <?php echo $ext; ?></span>
								...
								<tr>
									...
									<td>
										...
										?>
										<ul style="padding-left:40px;"><?php
								...
								<tr>
									<td colspan="3">
										<div style="margin: 20px 0 20px 200px;">
											...
											<input type="button" class="btn btn-warning" value="<?php echo $this->lng['cancel']; ?>" onclick="document.location.href='<?php echo $this->e2gModCfg['index'] . (!empty($this->sanitizedGets['tag']) ? '&amp;tag=' . $this->sanitizedGets['tag'] : '&amp;pid=' . $this->e2gModCfg['parent_id']); ?>'" />
										</div>
									</td>
								</tr>
							</table>
