    ...
    if (isset($this->sanitizedGets['page']) && $this->sanitizedGets['page'] == 'edit_plugin') {
        ...
        <div class="tab-page" id="tabPluginEdit">
            ...
            <form action="<?php echo $this->e2gModCfg['index']; ?>&amp;act=update_plugin" method="post">
                ...
                $query_plugins = mysqli_query($this->modx->db->conn,$select_plugins);
                if (!$query_plugins)
                    die(__LINE__ . ': ' . mysqli_errno($this->modx->db->conn) . ' ' . mysqli_error($this->modx->db->conn) . '<br />' . $select_plugins);
                else {
                    $numrow_plugins = mysqli_num_rows($query_plugins);
                    $row = mysqli_fetch_assoc($query_plugins);
    ...
    <!--                    <form action="<?php echo $this->e2gModCfg['index']; ?>&amp;act=save_plugin" method="post">-->
            <ul>
                ...
                if (!$query_plugins)
                    ...
                    while ($row = mysqli_fetch_assoc($query_plugins)) {
                        ?>
                        <li>
