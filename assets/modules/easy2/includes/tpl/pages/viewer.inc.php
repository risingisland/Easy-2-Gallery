...
<div class="tab-pane" id="tabThumbViewerPane">
     ...
     */ else {
        ...
        <div class="tab-page" id="tabThumbViewerSettings">
            ...
            $query_viewers = mysqli_query($this->modx->db->conn,$select_viewers);
            if (!$query_viewers)
                die(__LINE__ . ' : ' . mysqli_error($this->modx->db->conn) . '<br />' . $select_viewers);
            else {
                $numrow_viewers = mysqli_num_rows($query_viewers);
                ?>
                <ul>
                    <?php while ($row = mysqli_fetch_assoc($query_viewers)) {
