    ...
    public function process() {
        ...
        foreach ($fetchDirs as $fetchDir) {
            ...
            if (!empty($webGroupNames)) {
                ...
                $dirIcon .= '
                <i class="icon-custom icon-no-border icon-bg-purple fa fa-lock fa-2x icon-vs" title="' . $this->modx->e2gMod->lng['access'] . ': ' . $webGroupNames . '"></i>
                        ';
            }
        ...
        foreach ($fetchFiles as $fetchFile) {
            ...
            if (!empty($fetchFile['redirect_link'])) {
                $fileIcon .= '
                <i class="icon-custom icon-no-border icon-bg-blue fa fa-picture-o fa-2x icon-vs" title="' . $this->modx->e2gMod->lng['redirect_link'] . ': ' . $fetchFile['redirect_link'] . '"></i>
                        ';
            }
